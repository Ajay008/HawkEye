...
class MD_MT(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.fg_color = controller.color_dict['mt_fg_color']
        ...
        self.row += 1  #
        self.main_label = Label(self.settings_panel, text="Motion Tracker", bg=self.bg_color, fg=self.fg_color, font="Times 12 bold")
        ...
        #Specific
        self.row += 1  #
        Label(self.settings_panel, text="Motion Area : ", bg=self.bg_color, fg=self.fg_color).grid(row=self.row, column=0, pady=3)
        self.motion_area_value_scale = Scale(self.settings_panel, from_=0, to=400, bg=self.bg_color, highlightthickness=0, orient=HORIZONTAL, length=150, sliderlength=10, command=self.change_motion_area_value)
        self.motion_area_value_scale.set(self.motion_area_value)
        self.motion_area_value_scale.grid(row=self.row, column=1, ipady=10)
    ...
    #Specific
    def motion_tracking(self, temp, img):
        ...
        edged = cv2.Canny(img_temp_and_bgr,30,200)
        ...
        for c in sorted_contours:
            if cv2.contourArea(c) > self.motion_area_value:
    ...
    def change_motion_area_value(self, value):
        self.motion_area_value = int(value)
    ...
    def play(self):
        ...
        if available :
            ...
            new_image = self.motion_tracking("t1",imgCV_resized)

            imgTK2 = self.convert_imgCV_to_imgTK(new_image)         
